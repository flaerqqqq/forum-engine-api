...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Long> {
     ...
     * It returns an {@link Optional} containing the {@link RefreshToken} if found, or an empty {@link Optional} if not.
     * </p>
     *
     * @param userId the unique identifier of the user
     * @return an {@link Optional} containing the {@link RefreshToken} if found, otherwise an empty {@link Optional}
     */
    @Query("SELECT rt FROM RefreshToken rt WHERE rt.authUser.id = :userId")
    Optional<RefreshToken> findByUserId(String userId);

    /**
     * Finds a {@link RefreshToken} by its token value.
     * <p>
     * This method retrieves a refresh token based on its token string.
     * It returns an {@link Optional} containing the {@link RefreshToken} if found, or an empty {@link Optional} if not.
     * </p>
     *
